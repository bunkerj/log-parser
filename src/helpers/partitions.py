...
class Partitions:
    ...
    def add(self, log_indices, step_num):
        ...
        self._validate_log_lengths(log_indices)
    ...
    def _validate_log_lengths(self, log_indices):
        ref_token_idx = log_indices[0]
        ref_token_length = len(self.tokenized_logs[ref_token_idx])
        for token_idx in log_indices:
            token_length = len(self.tokenized_logs[token_idx])
            if token_length != ref_token_length:
                msg = 'Invalid log length ---> Expected: {} / Actual: {}'
                raise Exception(
